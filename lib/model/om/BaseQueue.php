...
/**
 * Base class that represents a row from the 'queue' table.
 *
 * 
 *
 ...
 */
abstract class BaseQueue extends BaseObject  implements Persistent {
	 ...
	 * @return     Queue The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = QueuePeer::ID;
		}
	...
	public function setVideoId($v)
	{
		...
		if ($this->video_id !== $v) {
			$this->video_id = $v;
			$this->modifiedColumns[] = QueuePeer::VIDEO_ID;
		}
	...
	public function setStatus($v)
	{
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = QueuePeer::STATUS;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = QueuePeer::NUM_COLUMNS - QueuePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Queue object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = QueuePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseQueue:delete:pre') as $callable)
			{
			...
			if ($ret) {
				QueuePeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseQueue:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseQueue:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseQueue:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				QueuePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = QueuePeer::doInsert($this, $con);
					...
					$affectedRows += QueuePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = QueuePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = QueuePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = QueuePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(QueuePeer::ID)) $criteria->add(QueuePeer::ID, $this->id);
		if ($this->isColumnModified(QueuePeer::VIDEO_ID)) $criteria->add(QueuePeer::VIDEO_ID, $this->video_id);
		if ($this->isColumnModified(QueuePeer::STATUS)) $criteria->add(QueuePeer::STATUS, $this->status);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(QueuePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Queue (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Queue Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getStatusRelatedByStatus(PropelPDO $con = null)
	{
		if ($this->aStatusRelatedByStatus === null && ($this->status !== null)) {
			$this->aStatusRelatedByStatus = StatusPeer::retrieveByPk($this->status);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aStatusRelatedByStatus->addQueues($this);
			 */
		}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseQueue:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseQueue::%s', $method));
	  }
